...
| `project_ready` | Supported direction, updated evidence, and confirmed target responsibilities | Dynamically generated primary and backup 3–7 day MVP cards plus role mapping | User accepts one feasible scope |
| `project_active` | Accepted MVP and daily availability | Full roadmap plus one expanded current step, checkpoints, tests, and fallback scope | Current artifact passes before the next step is expanded |
